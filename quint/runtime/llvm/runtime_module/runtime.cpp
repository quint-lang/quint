#if !defined(QUINT_INCLUDED) || !defined(_WIN32)
...
#include "quint/inc/cuda_kernel_utils.inc.h"
...
#if defined(__linux__) && defined(QUINT_ARCH_X64)
...
#define RUNTIME_STRUCT_FIELD(S, F)                                    \
  extern "C" void runtime_##S##_get_##F(LLVMRuntime *runtime, S *s) { \
    runtime->set_result(quint_result_buffer_runtime_query_id, s->F); \
  }

#define RUNTIME_STRUCT_FIELD_ARRAY(S, F)                                     \
  extern "C" void runtime_##S##_get_##F(LLVMRuntime *runtime, S *s, int i) { \
    runtime->set_result(quint_result_buffer_runtime_query_id, s->F[i]);     \
  }
...
void quint_printf(LLVMRuntime *runtime, const char *format, Args &&...args);
...
std::size_t quint_strlen(const char *str) {
    std::size_t len = 0;
    for (auto p = str; *p; p++)
        len++;
    return len;
...
struct PhysicalCoordinates {
    i32 val[quint_max_num_indices];
};

STRUCT_FIELD_ARRAY(PhysicalCoordinates, val) ;

#include "quint/program/context.h"
#include "quint/runtime/llvm/runtime_module/mem_request.h"
...
#include "quint/runtime/llvm/runtime_module/atomic.h"
...
void quint_assert(RuntimeContext *context, i32 test, const char *msg);
void quint_assert_runtime(LLVMRuntime *runtime, i32 test, const char *msg);
#define QUINT_ASSERT_INFO(x, msg) quint_assert(context, (int)(x), msg)
#define QUINT_ASSERT(x) QUINT_ASSERT_INFO(x, #x)
...
T debug_add(RuntimeContext *ctx, T a, T b, const char *tb) {
  ...
  if (__builtin_add_overflow(a, b, &c)) {
    quint_printf(ctx->runtime, "Addition overflow detected in %s\n", tb);
  }
  return c;
}
...
T debug_sub(RuntimeContext *ctx, T a, T b, const char *tb) {
  ...
  if (__builtin_sub_overflow(a, b, &c)) {
    quint_printf(ctx->runtime, "Subtraction overflow detected in %s\n", tb);
  }
  return c;
}
...
T debug_mul(RuntimeContext *ctx, T a, T b, const char *tb) {
  ...
  if (__builtin_mul_overflow(a, b, &c)) {
    quint_printf(ctx->runtime, "Multiplication overflow detected in %s\n", tb);
  }
  return c;
}
...
T debug_shl(RuntimeContext *ctx, T a, i32 b, const char *tb) {
  T c = a << b;
  if (c >> b != a) {
    quint_printf(ctx->runtime, "Shift left overflow detected in %s\n", tb);
  }
  return c;
}
...
struct ListManager {
    ...
    ListManager(LLVMRuntime *runtime,
            ...
            : element_size(element_size),
              ...
              runtime(runtime) {
        quint_assert_runtime(runtime, is_power_of_two(max_num_elements_per_chunk),
                              "max_num_elements_per_chunk must be POT.");
        lock = 0;
        num_elements = 0;
        log2chunk_num_elements = quint::log2int(num_elements_per_chunk);
    }
    ...
    i32 ptr2index(Ptr ptr) {
        ...
        for (int i = 0; i < max_num_chunks; i++) {
            quint_assert_runtime(runtime, chunks[i] != nullptr, "ptr not found.");
...
struct LLVMRuntime {
    ...
    ListManager *element_lists[quint_max_num_snodes];
    NodeManager *node_allocators[quint_max_num_snodes];
    NodeManager *runtime_context_buffer_allocator;
    Ptr ambient_elements[quint_max_num_snodes];
    Ptr temporaries;
    ...
    char error_message_template[quint_error_message_max_length];
    uint64 error_message_arguments[quint_error_message_max_arguments];
    ...
    void set_result(std::size_t i, T t) {
        ...
        ((u64 *)result_buffer)[i] =
                quint_union_cast_with_different_sizes<uint64>(t);
    }
...
void RuntimeContext_store_result(RuntimeContext *ctx, u64 ret, u32 idx) {
    ctx->result_buffer[quint_result_buffer_ret_value_id + idx] = ret;
}
...
void runtime_retrieve_and_reset_error_code(LLVMRuntime *runtime) {
    runtime->set_result(quint_result_buffer_error_id, runtime->error_code);
    runtime->error_code = 0;
}

void runtime_retrieve_error_message(LLVMRuntime *runtime, int i) {
    runtime->set_result(quint_result_buffer_error_id,
...
void runtime_ListManager_get_num_active_chunks(LLVMRuntime *runtime,
    ...
    runtime->set_result(quint_result_buffer_runtime_query_id,
...
void quint_assert(RuntimeContext *context, i32 test, const char *msg) {
    quint_assert_runtime(context->runtime, test, msg);
}

void quint_assert_format(LLVMRuntime *runtime,
                          i32 test,
                          const char *format,
                          int num_arguments,
                          uint64 *arguments) {
        ...
        locked_task(&runtime->error_message_lock, [&] {
            if (!runtime->error_code) {
                ...
                memset(runtime->error_message_template, 0,
                       quint_error_message_max_length);
                memcpy(runtime->error_message_template, format,
                       std::min(quint_strlen(format),
                                quint_error_message_max_length - 1));
  ...
  asm("exit;");
    ...
    // (Aborted), which will be caught by Quint's signal handler. The assert
    // failure message will NOT be properly printed since Quint exits after
...
void quint_assert_runtime(LLVMRuntime *runtime, i32 test, const char *msg) {
    quint_assert_format(runtime, test, msg, 0, nullptr);
}
...
Ptr LLVMRuntime::allocate_from_buffer(std::size_t size, std::size_t alignment) {
    ...
    // halt the whole grid. Using a quint_assert_runtime will not finish the
    ...
    __assertfail(
        ...
        " GPU memory",
        "Quint JIT", 0, "allocate_from_buffer", 1);
#endif
    }
    quint_assert_runtime(this, success, "Out of pre-allocated memory");
    return ret;
}
...
Ptr LLVMRuntime::request_allocate_aligned(std::size_t size,
    ...
    if (preallocated)
        ...
        quint_assert_runtime(this, i <= quint_max_num_mem_requests,
...
void runtime_get_mem_req_queue(LLVMRuntime *runtime) {
    runtime->set_result(quint_result_buffer_ret_value_id,
...
void runtime_initialize(
    ...
    if (preallocated_size) {
        ...
        preallocated_buffer +=
                quint::iroundup(sizeof(LLVMRuntime), quint_page_size);
    } else {
        runtime =
    ...
    runtime->set_result(quint_result_buffer_ret_value_id, runtime);
...
void runtime_initialize_snodes(LLVMRuntime *runtime,
    ...
    for (int i = 0; i < quint_max_num_indices; i++) {
        elem.pcoord.val[i] = 0;
    }
...
void element_listgen_root(LLVMRuntime *runtime,
  ...
  int c_step = grid_dim() * block_dim();
    ...
    auto ch_element_size =
            std::min(ch_num_elements, quint_listgen_max_element_size);
...
void cpu_parallel_range_for(RuntimeContext *context,
    ...
    ctx.step = step;
    if (step != 1 && step != -1) {
        quint_printf(context->runtime, "step must not be %d\n", step);
        exit(-1);
    }
...
void ListManager::touch_chunk(int chunk_id) {
    quint_assert_runtime(runtime, chunk_id < max_num_chunks,
...
void gc_parallel_impl_0(RuntimeContext *context, NodeManager *allocator) {
    ...
    if (free_list_used * 2 > free_list_size) {
        // Directly copy. Dst and quint does not overlap
...
f32 rounding_prepare_f32(f32 f) {
    ...
    */
    ...
    i32 delta_bits =
            (quint_union_cast<i32>(f) & 0x80000000) | quint_union_cast<i32>(0.5f);
    f32 delta = quint_union_cast<f32>(delta_bits);
    return f + delta;
}

f64 rounding_prepare_f64(f64 f) {
    // Same as above
    i64 delta_bits = (quint_union_cast<i64>(f) & 0x8000000000000000LL) |
            quint_union_cast<i64>(0.5);
    f64 delta = quint_union_cast<f64>(delta_bits);
    return f + delta;
}
}
