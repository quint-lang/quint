...
//
#include "quint/program/parallel_executor.h"
...
namespace quint::lang {
    ...
    void ParallelExecutor::enqueue(const ParallelExecutor::TaskType &func) {
        if (num_threads_ <= 0) {
            func();
            return;
        }
    ...
    void ParallelExecutor::worker_loop() {
        QUINT_DEBUG("Starting worker thread.");
        ...
        QUINT_DEBUG("Worker thread initialized and running.");
        bool done = false;
        while (!done) {
