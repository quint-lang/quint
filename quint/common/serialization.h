...
#ifndef QUINT_SERIALIZATION_H
#define QUINT_SERIALIZATION_H
...
#include "quint/common/json.h"
#include "quint/common/json_serde.h"
#include "quint/common/core.h"
#include "quint/common/interface.h"

#ifdef QUINT_INCLUDED
namespace quint {
#else
#define TI_TRACE
#define TI_CRITICAL
...
#define QUINT_IO(...)                                                  \
  do {                                                                 \
    ...
    inline std::vector<uint8_t> read_data_from_file(const std::string &fn) {
        ...
        if (f == nullptr) {
            QUINT_DEBUG("Cannot open file: {}", fn);
    ...
    inline void write_data_to_file(const std::string &fn,
        ...
        if (f == nullptr) {
            QUINT_ERROR("Cannot open file [{}] for writing. (Does the directory exist?)",
                     fn);
            assert(f != nullptr);
        }
        if (ends_with(fn, ".tcb.zip")) {
            ...
            QUINT_ERROR("File must end with .tcb or .tcb.zip. [Filename = {}]", fn);
        }
    }
    ...
    public:
        ...
        typename std::enable_if<writing_, bool>::type initialize(
            ...
            head = 0;
            if (preserved_ != 0) {
                QUINT_TRACE("preserved = {}", preserved_);
                // Preserved mode
    ...
    private:
            ...
            if (writing) {
                ...
                if (c_data) {
                    if (new_size > preserved) {
                        QUINT_CRITICAL("Preserved Buffer (size {}) Overflow.", preserved);
                    }
        ...
        typename std::enable_if<std::is_pointer<T>::value, void>::type process(
            ...
            if (writing) {
                ...
                if (val_ptr != 0) {
                    QUINT_ASSERT(assets.find(val_ptr) != assets.end());
