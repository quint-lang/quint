...
set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DQUINT_ISE_NONE")

option(BUILD_WITH_ADDRESS_SANITIZER "Builder with clang address sanitizer" OFF)
...
if (WIN32)
    link_directories(${CMAKE_CURRENT_SOURCE_DIR}/tpl/lib)
    if (MSVC)
    ...
    elseif ("${CMAKE_CXX_COMPILER_ID}" MATCHES "GNU")
        ...
        message(FATAL_ERROR "clang and MSVC are the only supported compilers for Quint compiler development. Consider using 'cmake -DCMAKE_CXX_COMPILER=clang' if you are on Linux.")
    endif()
...
if ("${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "x86_64" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "AMD64" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "amd64")
    if (MSVC)
        set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} /D \"QUINT_ARCH_x64\"")
    else()
        set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DQUINT_ARCH_x64")
...
elseif ("${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "aarch64" OR "${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "arm64")
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DQUINT_ARCH_ARM")
    set(ARCH "arm64")
elseif ("${CMAKE_SYSTEM_PROCESSOR}" STREQUAL "x86")
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DQUINT_ARCH_x86")
    set(ARCH "x86")
else()
...
set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DQUINT_PASS_EXCEPTION_TO_PYTHON")

set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DQUINT_INCLUDED")

if ($ENV{QUINT_USE_DOUBLE})
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DQUINT_USE_DOUBLE")
...
if (QUINT_USE_MPI)
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DQUINT_USE_MPI")
    message("Using MPI")
endif ()